...
class Transition(Scene):
    ...
    def __init__(self, name: str, to_scene: Scene, from_scene: Scene):
        ...
        :param to_scene: The scenes that the transition is transitioning to.
        :param from_scene: The scenes that the transition is transitioning from.
        """
        super().__init__(name)
...
class CircularTransition(Transition):
    ...
    def update(self):
        """
        Update the circle radius and transition to the next scenes if necessary.
        """
        if self.transitioningIn:
            ...
            if self.circleRadius >= self.exitCircleRadius:
                scene_manager.set_active_scene(self.toScene, swap=True)
